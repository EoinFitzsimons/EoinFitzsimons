...
(function(){
  ...
  function apply(y){
    if (!visible && y > SHOW_Y){ visible = true; document.body.classList.add('side-visible'); }
    else if (visible && y < HIDE_Y){ visible = false; document.body.classList.remove('side-visible'); }
  }
  function onScroll(){
    const y = window.scrollY || window.pageYOffset || 0;
    if (!ticking){ requestAnimationFrame(()=>{ apply(y); ticking = false; }); ticking = true; }
  }
...
(function(){
  ...
  if ('IntersectionObserver' in window){
    ...
    sectionIds.forEach(id=>{ const el = document.getElementById(id); el && io.observe(el); });
  }
})();

// ===== Copy helpers =====
(function(){
  function copy(val){ navigator.clipboard?.writeText(val).then(()=> live('Copied to clipboard')); }
  $('#copyPhone')?.addEventListener('click', ()=>copy('+353894601090'));
  $('#copyEmail')?.addEventListener('click', ()=>copy('Eoin00Fitzsimons@gmail.com'));
  $('#copyEmail2')?.addEventListener('click', ()=>copy('Eoin00Fitzsimons@gmail.com'));
...
(function(){
  ...
  function pagesUrl(repo){ return `https://${ghUser.toLowerCase()}.github.io/${repo}/`; }
  function repoUrl(repo){ return `https://github.com/${ghUser}/${repo}`; }
  function previewUrl(repo){ return `https://htmlpreview.github.io/?https://github.com/${ghUser}/${repo}/blob/main/index.html`; }
  function chip(label){ const span=document.createElement('span'); span.className='chip'; span.textContent=label; return span; }
  ...
  function openModal(title, url, fallback){
    ...
    frame.src = url; openNewTab.href = url;
  ...
  document.addEventListener('click', (e)=>{
    ...
    if (btn){
      ...
      if (window.matchMedia('(max-width: 860px)').matches){ window.open(url, '_blank', 'noopener'); }
      else { openModal(title, url, fallback); }
    }
    if (e.target === modal || e.target === closeModal){ hideModal(); }
  });
